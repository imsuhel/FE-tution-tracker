...
import { ArrowLeft, GripVertical, Plus, Trash2, BookOpen, Save } from "lucide-react";
import { Card } from "@/components/ui/Card";
import { updateCourse, addCourseModule, deleteCourseModule } from "@/lib/actions/course.actions";
...
export default function CourseDetailsClient({ 
  ...
  const handleAddModule = async (e: React.FormEvent) => {
    ...
    setNewModuleName(""); // Clear input early for snappy feel
    ...
    if (result.success) {
      // Replace optimistic module with real one from server
      setModules(prev => prev.map(m => m.id === tempId ? result.module : m));
    } else {
      // Rollback
      setModules(previousModules);
      setNewModuleName(moduleNameToAdd); // Put name back in input
      alert(result.error);
    }
  };
  ...
  const handleSaveCourse = async () => {
    ...
    // The state is already updated as the user types, so we just call the API
  ...
  return (
    <div className="flex flex-col gap-8 max-w-5xl mx-auto pb-12">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-neutral-100 mb-1">Course Management</h1>
          <p className="text-sm text-neutral-400">Manage course details and curriculum modules.</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input 
                ...
                onChange={(e) => setCourse({...course, monthly_fee: parseInt(e.target.value) || 0})}
                className={`w-full ${inputClass}`} 
              />
            </div>
          </div>
      ...
      <Card className="flex flex-col border-neutral-700/50 bg-[#2b2b2b] overflow-hidden">
        <div className="p-4 border-b border-neutral-800 bg-[#262626] flex justify-between items-center">
          <div>
            <h2 className="text-lg font-bold text-neutral-100">Curriculum Modules</h2>
            <p className="text-sm text-neutral-400 mt-1">Define the topics covered in this course in order.</p>
          </div>
          <button 
        ...
        <div className="p-4 border-b border-neutral-800 bg-[#1e1e1e]">
          <form onSubmit={handleAddModule} className="flex gap-3">
            ...
            <button 
              ...
              className="flex items-center gap-2 bg-[#2b2b2b] text-neutral-200 border border-neutral-700 rounded-lg px-4 py-2 text-sm font-medium hover:bg-neutral-700 transition-colors disabled:opacity-50"
            >
              ...
              >
                ...
                {!module.isOptimistic && (
                  <button 
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1.5 text-neutral-500 hover:text-red-400 hover:bg-red-900/20 rounded transition-all"
                    title="Remove Module"
                  >
